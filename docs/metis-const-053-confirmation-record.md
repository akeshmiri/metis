...
4. **What does ZDR guarantee, precisely?** Confirm: no storage of inputs/outputs beyond what's needed for abuse-screening; User Safety classifier results are still retained even under ZDR (this is normal, not a gap — confirm the adopting org accepts this as expected, not a surprise later).
...
- A rough estimate of ingestion volume (already computed: `metis-cost-review-15k-tests.md`'s reference figures — 15,000+ tests, 100,000 Jira tickets — a vendor sales team will want a usage-scale picture, not just "we want ZDR")
