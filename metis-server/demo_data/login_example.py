...
# Each entry: (transition_id, from_state, to_state, trigger, guard,
#              intent_text, requirement_text, ac_texts, techniques,
#              test_levels, implementation_status, risk_tag, perf_critical)
_TRANSITIONS = [
    (
        "t1-valid-login", "LoggedOut", "LoggedIn", "submit_valid_credentials",
    ...
    (
        "t2a-invalid-login-attempt-1", "LoggedOut", "Failed1", "submit_invalid_credentials",
        ...
        ["unit", "api_functional"],
        "implemented", "Low", False,
    ),
    (
        "t2b-invalid-login-attempt-2", "Failed1", "Failed2", "submit_invalid_credentials",
        "NOT credentials_valid",
    ...
    (
        "t2c-invalid-login-attempt-3", "Failed2", "Failed3", "submit_invalid_credentials",
        "NOT credentials_valid",
    ...
    (
        "t2d-invalid-login-attempt-4", "Failed3", "Failed4", "submit_invalid_credentials",
        "NOT credentials_valid",
        ...
        ["unit", "api_functional"],
        "implemented", "Medium", False,
    ),
    (
        "t3-lockout", "Failed4", "AccountLocked", "submit_invalid_credentials",
        "NOT credentials_valid",
    ...
    (
        "t1b-valid-login-after-1-failure", "Failed1", "LoggedIn", "submit_valid_credentials",
        "credentials_valid",
    ...
    (
        "t1c-valid-login-after-2-failures", "Failed2", "LoggedIn", "submit_valid_credentials",
        "credentials_valid",
    ...
    (
        "t1d-valid-login-after-3-failures", "Failed3", "LoggedIn", "submit_valid_credentials",
        "credentials_valid",
    ...
    (
        "t1e-valid-login-after-4-failures", "Failed4", "LoggedIn", "submit_valid_credentials",
        "credentials_valid",
    ...
    (
        "t4-forgot-password", "LoggedOut", "PasswordResetRequested", "click_forgot_password", "true",
    ...
    (
        "t5-reset-email-sent", "PasswordResetRequested", "PasswordResetSent", "submit_reset_email",
        "email_registered",
    ...
    (
        "t6-session-timeout", "LoggedIn", "SessionExpired", "session_idle_timeout",
    ...
    (
        "t7-expired-redirect", "SessionExpired", "LoggedOut", "any_action_after_expiry", "true",
    ...
    (
        "t8-account-unlock", "AccountLocked", "LoggedOut", "admin_unlock_or_lockout_elapsed",
    ...
    (
        "t9-logout", "LoggedIn", "LoggedOut", "click_logout", "true",
    ...
    (
        "t10-2fa-enrollment", "LoggedIn", "LoggedIn", "enroll_2fa", "true",
...
def build_login_example(session, r: random.Random, episode_fn, DEMO_TAG: str,
    ...
    for (tid, from_state, to_state, trigger, guard, intent_text, req_text, ac_texts,
         techniques, test_levels, impl_status, risk_tag, perf_critical) in _TRANSITIONS:
        ...
        session.run(
            ...
            "t.is_demo_data = true, t.source_kind = 'behavior_example'",
            id=transition_id, status=impl_status,
        ).consume()
        for state_id in (f"demo:login:state:{from_state}", f"demo:login:state:{to_state}"):
            session.run(
                ...
                "s.is_demo_data = true, s.source_kind = 'behavior_example'",
                id=state_id, status=impl_status,
            ).consume()
        ...
        add_nodes("Requirement", [{
            ...
            "ears_pattern": ears.pattern, "revision": 1, "corroboration_count": 2,
            "risk_tag": risk_tag, "lifecycle_state": tier_result.lifecycle_state,
        ...
        for j, ac_text in enumerate(ac_texts):
            ...
            add_nodes("AcceptanceCriterion", [{
                ...
                "revision": 1, "source_kind": "behavior_example", DEMO_TAG: True,
            }])
