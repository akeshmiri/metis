...
| `RetentionPolicy` | the **complexity condition**: `retentionDays` nests conditions and short-circuits to McCabe 14, `isRetained` is straight-line at 1. Nothing else in this service branches more than three times, so without it an extracted complexity metric is flat — and a flat metric is one a test cannot tell from a broken one |
